 ...
 * @LastEditors: Cyan
 * @LastEditTime: 2023-05-24 13:44:21
 */
'use strict';
...
class HomeController extends Controller {
    ...
    async login() {
        ...
        try {
            ...
            const token = jwt.sign(payload, this.config.privateKey, { expiresIn: this.config.expiresIn }); // 生成token
            // 判断验证码是否正确
            ...
            if (results.length) {
                ...
                if (v.Status == 1) {
                    ...
                    const roleList = JSON.parse(v.roleList).join(',');
                    let roleArr = [],
                        permissionArr = [];
            ...
            ctx.body = { state: 0, msg: '登录失败!', error };
        }
    }
    ...
    async logout() {
        ...
        const { Raw } = app.Db.xmw;
        try {
            const { UserID, username, CnName } = ctx.session.userInfo;
            const logoutInfo = { token: '' };
            const options = {
            ...
            ctx.body = { state: 0, msg: '注销失败!', error };
        }
    }
    ...
    async updateUserInfo() {
        ...
        try {
            ...
            ctx.body = { state: 0, msg: '保存失败!', error };
        }
    }
    ...
    async changeUserPassword() {
        ...
        try {
            const { UserID, username, CnName } = ctx.session.userInfo;
            const passwordParams = ctx.request.body;
            const result = await Raw.Query(`select password from xmw_user where UserID=${UserID}`);
            if (passwordParams.originalPassword == result.password) {
                const row = { password: passwordParams.lastPassword };
                const options = {
                    wherestr: `where UserID=${UserID}`,
                };
                await Raw.Update('xmw_user', row, options);
    ...
    async changeUserLabel() {
        ...
        try {
            const { UserID, username, CnName } = ctx.session.userInfo;
            const labelParams = ctx.request.body;
    ...
    async changeUserImg() {
        ...
        try {
            ...
            const filename = ctx.helper.dataFormat(new Date(), 'yyyyMMddhhmmss') + '_' + Math.floor(Math.random() * 10000 + 1000) + fileExt;
            ...
            const basePath = 'public/' + SaveFileName + '/' + filename;
            ...
            ctx.body = { state: 0, msg: '上传失败!', error };
        }
    }
    ...
    async getUserPw() {
        ...
        try {
            ...
            const password = await Raw.Query(`select password from xmw_user where UserID = '${UserID}'`);
            ctx.body = { state: 1, msg: '请求成功!', result: password };
        } catch (error) {
            ctx.logger.info('getUserPw方法报错：' + error);
            ctx.body = { state: 0, msg: '请求失败!', error };
        }
    }
