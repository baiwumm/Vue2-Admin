<template>
  <page-header-wrapper>
    <a-card :bordered="false">
      <!-- 顶部搜索 -->
      <header-search :query-form="queryForm" @query="query" @onAdd="onAdd" :orgList="orgList" />
      <!-- 表格数据 -->
      ...
      <a-drawer :title="title" :width="400" :visible="visible" :maskClosable="false" @close="onClose">
        <a-form :form="form">
          <form-drawer :rules="rules" :data="data" :orgList="orgList" />
        </a-form>
        <div
          :style="{
            position: 'absolute',
...
import { addPost, delPost, getPostList, updatePost } from '@/api/administrative/post'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        parentId: ['parentId'],
        name: ['name', { rules: [{ required: true, message: I18nEntry(I18nPost('name')) }] }],
        orgId: ['orgId', { rules: [{ required: true, message: I18nSelect(I18nPost('orgId')) }] }],
        description: ['description', { rules: [{ required: true, message: I18nEntry(I18nPost('description')) }] }],
  ...
  methods: {
    ...
    async getList() {
      this.loading = true
      const params = pick(this.queryForm, ['orgId', 'name'])
      ...
      const { data, code } = await getPostList(params)
      if (code === RequestCode.Success) {
    ...
    onAdd() {
      this.visible = true
      this.title = `${this.$t(I18nGlobal.Add)}${I18nPost()}`
    },
    // 编辑
    onEdit(record) {
      this.visible = true
      this.title = `${this.$t(I18nGlobal.Edit)}${I18nPost()}：${record.name}`
      this.id = record.id
    ...
    async onDelete(record) {
      await delPost(record.id).then((res) => {
    ...
    handleSubmit(e) {
      ...
      this.form.validateFields(async (err, { menus, ...values }) => {
        if (!err) {
          ...
          await (id ? updatePost : addPost)(params)
            .then(async ({ code, msg }) => {
              if (code === RequestCode.Success) {
